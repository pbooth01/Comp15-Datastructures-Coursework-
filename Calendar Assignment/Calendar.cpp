...
Day *Calendar::getDay(date startDate) {
	...
	//locates the matching day and returns its address in the array
...
// This function returns the address of the newly inserted day so other functions can insert events
// into that Day.
Day *Calendar::insertDay(date startDate) {
	...
	for(i =0; i < dayCount; i ++){
		if (newDay->isBefore(dayList[i])){
			break;
		}
	}
	temp2 = i;
	for(int j = dayCount; j > temp2; j--){
		dayList[j] = dayList[j-1];
	}
		dayList[temp2] = *newDay;
		dayCount++;
		return &dayList[temp2];
}
